...
namespace VR {
    ...
    public class AvatarBodySync : NetworkBehaviour {
        ...
        private Vector3 _headToMainBone;
        private float _avatarScale = 1;

        // Boolean used when the body is rotating towards the head y rotation
        private bool _rewind;

        void Awake() {
            _headToMainBone = pelvisBone.position - headBone.position;
        }
        ...
        /// </summary>
        void SyncHead() {
            if(XROriginRigReferences.Instance == null) return;
            
            Transform vrHead = XROriginRigReferences.Instance.head;

            Vector3 scaledHeadsetOffset = headsetOffset * _avatarScale;
            
            headBone.position = vrHead.position + vrHead.up*scaledHeadsetOffset.y + vrHead.right*scaledHeadsetOffset.x + vrHead.forward*scaledHeadsetOffset.z;
            headBone.rotation = vrHead.rotation * Quaternion.Euler(0,180,0);
        }
        ...
        void BodyIK() {
            ...
            Vector3 headToMainBoneUpdated = Vector3.Scale(_headToMainBone*_avatarScale, new Vector3(1, 1, 10*leaningValue));
        ...
        void SyncHands() {
            ...
            SyncTransform(leftHandRootBone, XROriginRigReferences.Instance.leftHand.rootBone, true, Side.Left);
            SyncTransform(rightHandRootBone, XROriginRigReferences.Instance.rightHand.rootBone, true, Side.Right);
            ...
            for (int i = 0; i < leftHandRootBone.childCount; i++) {
                ...
                SyncBoneRec(leftHandRootBone.GetChild(i), FindBoneByName(XROriginRigReferences.Instance.leftHand.rootBone, fingerName), Side.Left);
            }

            for (int i = 0; i < rightHandRootBone.childCount; i++) {
                string fingerName = rightHandRootBone.GetChild(i).name.Split('_')[0];

                SyncBoneRec(rightHandRootBone.GetChild(i), FindBoneByName(XROriginRigReferences.Instance.rightHand.rootBone, fingerName), Side.Right);
            }
        }
