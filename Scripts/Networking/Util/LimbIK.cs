...
namespace Util {
    ...
    public class LimbIK : MonoBehaviour {
        ...
        void Start() {
            _elbow = transform.parent;
            _shoulder = _elbow.parent;
        ...
        void FixedUpdate() {
            ...
            Vector3 shoulderPosition = _shoulder.position;
            ...
            _shoulder.LookAt(targetPosition, armNormal);
            _shoulder.localRotation *= Quaternion.Euler(90, 0, 0);
            _shoulder.localRotation *= Quaternion.Euler(0, 90, 0);

            if (shoulderToTargetLength > _limbsLength) {
                // If the target is too far for our limbs we just reach as far as possible
                _elbow.localRotation = Quaternion.Euler(0, 0, 0);
            } else {
                ...
                _shoulder.localRotation *= Quaternion.Euler(shoulderAngle, 0, 0);
                _elbow.localRotation = Quaternion.Euler(elbowAngle+180, 0, 0);
            }
        ...
        public void ComputeLength() {
            Vector3 elbowPosition = _elbow.position;
            Vector3 shoulderPosition = _shoulder.position;
