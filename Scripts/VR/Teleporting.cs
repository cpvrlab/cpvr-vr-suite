...
namespace cpvr_vr_suite.Scripts.VR
{
    public class Teleporting : MonoBehaviour
    {
        ...
        private void DisableTeleport()
        {
            if (!_teleport) return;
            _teleport = false;
            if (_teleportPosition == Vector3.zero /*|| PalmFacesHead()*/) return;   // Palm direction check disabled due to a current bug with the meta aim hand position
        ...
        private void CastRay(Vector3[] p, out int lastPoint, out Vector3 hitPos, out Vector3 hitNormal, out bool valid)
        {
            ...
            hitPos = hit.point;
            hitNormal = hit.normal;
            valid = ((1 << hit.collider.gameObject.layer) & teleportLayer) != 0 /*&& !PalmFacesHead()*/;    // Disable due to bug
        }
